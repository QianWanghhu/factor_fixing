...
nstart, nstop, nstep = 10000, 12000, 500
...
for n in range(nstart, nstop + 1, nstep):
    ...
    y_uncond[str(n)] = poly(x_sample).flatten()
        # conf_uncond[str(n)] = np.quantile(y_uncond[key], [0.025, 0.975])
    error_dict[str(n)], pool_res = group_fix(value, poly, x_sample, y_uncond[str(n)], x_fix, rand, {}, file_exist=True)
    # End for
...
for ele in f_names:
    ...
    df.to_csv(f'{save_path}/{ele}_adaptive2.csv')